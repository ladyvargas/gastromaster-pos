...
app.post("/api/auth/login", async (req, res, next) => {
  try {
    ...
    if (!user || !user.active)
      throw badRequest("INVALID_CREDENTIALS", "Credenciales inválidas");
  ...
  async (req, res, next) => {
    try {
      ...
      if (order.status === "PAID" || order.status === "CANCELLED")
        throw badRequest("ORDER_CLOSED", "Pedido cerrado");
      ...
      const updated = await prisma.$transaction(async (tx) => {
        ...
        const totalCents = items.reduce(
          (acc, it) => acc + it.qty * it.priceCents,
          0
        );
...
app.patch(
  ...
  requireAuth,
  requireRole("ADMIN", "KITCHEN", "CASHIER", "WAITER"),
  async (req, res, next) => {
    try {
      const orderId = Number(req.params.id);
      const body = validate(z.object({ status: z.string().min(1) }), req.body);
